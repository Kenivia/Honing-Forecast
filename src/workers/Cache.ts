...
export function reconstruct1DTo2D(
  flat: Uint32Array | ArrayLike<number>,
  n: number,
  opts: { zeroCopy?: boolean } = { zeroCopy: true }
): Uint32Array[] {
...
export async function saveTypedArray(
  key: string,
...
export async function loadTypedArray(
  key: string,
  dbName = 'my-cache-db'
): Promise<{ arr: TypedArray; meta: any } | null> {
  const db = await openDB(dbName);
  return new Promise((resolve, reject) => {
