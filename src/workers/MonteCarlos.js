    ...
    while (trials <= count_limit) {
        ...
        if (Math.max(...adv_counts[0]) + Math.max(...adv_counts[1]) > 0) {
            for (const piece of [...Array(adv_hone_chances.length).keys()]) {
                let taps = [...Array(adv_hone_chances[piece].length).keys()].map((x) => x + 0)
                let tap;
                if (rigged) {
                    if (rigged_taps.length > 0) {
                        tap = rigged_taps[piece]
                    ...
                    tap = weighted_random(taps, adv_hone_chances[piece])
                }
                for (const cost_type of [...Array(adv_hone_costs[0].length).keys()]) {
                    cur_costs[cost_type] += adv_hone_costs[piece][cost_type][tap - 1]
                    // return adv_hone_costs[piece]
                    // cur_costs[0] = tap
                    // cur_costs[3] = adv_hone_chances.length
                }
            }
        }
