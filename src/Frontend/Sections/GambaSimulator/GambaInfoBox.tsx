...
import { Upgrade, calculateCurrentChance, getTapCountRange } from "@/Frontend/Utils/Helpers.ts"
    ...
    return (
        ...
        >
            <h4 style={{ margin: 0, marginBottom: 10 }}>
                {upgradeArr[selectedUpgradeIndex] === undefined ? "" : upgradeArr[selectedUpgradeIndex].is_normal_honing ? "+" : "Adv +"}
            ...
            {selectedUpgradeIndex !== null && upgradeArr[selectedUpgradeIndex] ? (
                <>
                    {upgradeArr[selectedUpgradeIndex].is_normal_honing ? (
                        <>
                            ...
                            <div>
                                For this upgrade, in a room of 100 people, you are luckier than{" "}
                                {(100 - (upgradeArr[selectedUpgradeIndex].cumulative_chance || 0) * 100).toFixed(0)} of them.
                            </div>
                        </>
                    ) : (
                        (() => {
                            ...
                            return rangeInfo ? (
                                <>
                                    <div>Tap Count Range: {rangeInfo.range}</div>
                                    <div>
                                        {"For this upgrade, in a room of 100 people, you are luckier than " +
                                            (100 - upgradeArr[selectedUpgradeIndex].cumulative_chance * 100).toFixed(0) +
                                            " of them."}
                                    </div>
                                </>
                            ) : null
                    ...
                    <div style={{ marginTop: 15, display: "flex", flexDirection: "column", gap: 8 }}>
                        ...
                        <button
                            ...
                            style={{
                                ...
                                opacity:
                                    isAutoAttempting || upgradeArr[selectedUpgradeIndex]?.is_finished || !upgradeArr[selectedUpgradeIndex]?.is_normal_honing
                                        ? 0.5
                                        : 1,
                            }}
                        >
                        ...
                        <button
                            onClick={freeTap}
                            disabled={
                                upgradeArr[selectedUpgradeIndex]?.is_normal_honing === false ||
                ...
                <div style={{ color: "var(--text-secondary)", fontStyle: "italic" }}>Select an upgrade to begin</div>
            )}
        </div>
    )
}
