...
export default function HoningForecastUI() {
    ...
    const columnDefs = useMemo(() => [{ headerName: 'Value', field: 'value', editable: true, flex: 1, cellStyle: { background: '#072f24', color: '#9ff7c7', padding: '6px 8px' } }], [])
    return (
        ...
        <div style={styles.pageContainer}>
            ...
            <div style={styles.mainContainer}>
                ...
                {/* Normal Honing Section */}
                <div style={{ display: 'flex', gap: 'var(--spacing-2xl)', alignItems: 'flex-start', flexDirection: "column" }}>
                    <h2 style={{ ...styles.sectionTitle, marginBottom: '-8px' }}>Normal Honing</h2>
                            ...
                            <div style={{ width: 100, display: 'flex', flexDirection: 'column', justifyContent: "flex-start", textWrap: "nowrap", gap: 0 }}>
                                {["", 'Helmet', 'Shoulder', 'Chest', 'Pants', 'Glove', 'Weapon'].map((lab) => (
                                    <div key={lab} style={{ height: 28, color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', paddingRight: 8 }}>
                                        {lab ? <Icon name={lab} size={28} style={{ fontSize: 'var(--font-size-sm)' }} /> : ''}
                                    </div>
                                ))}
                            </div>
