...
import { remapCountsToLockedXAxis } from "@/Frontend/Utils/HistogramUtils.ts"
...
function Graph({
    ...
    const handleSeriesPointerMove = useCallback(
        (idx: number) => () => {
            setHoverSeries(idx)
        },
        [setHoverSeries]
    ...
    const data_size =
        Math.max(...Array.from({ length: labels.length }, (_, i) => (effectiveCounts ?? counts)?.[i]?.reduce((partialSum, a) => partialSum + a, 0) ?? 0)) || 1
    ...
    const bottomTickFormat = useCallback(
        (val: any) => {
    ...
    const lineSeriesNodes = useMemo(() => {
        return sortedVisibleIndices.map((seriesIdx) => {
            ...
            const isDotted = (graphType === "Raw" && !isFirstSeriesInOriginalOrder) || (graphType === "Gold" && seriesIdx >= 2)
            ...
            labels.forEach((_, i) => {
                ...
                elems.push(
                    <g key={`${keyPrefix}-${i}`}>
                        ...
                        <text x={cx + 10} y={cy - 8} fill={effectiveColors[i]} fontSize={12}>
                            {labelText}
                        </text>
                    </g>
                )
    ...
    const hoverMarker = useMemo(() => {
        ...
        if (graphType === "Gold") {
            ...
            const hoverSeriesVals =
                cumulative && cdfSeries ? cdfSeries[fallbackSeries] : (normalizedCounts && normalizedCounts[fallbackSeries]) || srcCounts[fallbackSeries]
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        {lockXAxis && effectiveMaxs && fallbackSeries != null ? (
                            ...
                            >
                                ...
                                <span style={{ color: effectiveColors[fallbackSeries] }}>
                                    {labels[fallbackSeries].padEnd(Math.max(...labels.map((l) => l.length)), " ")}
                                </span>
                                {hasUnplottedPoints && <span>- Some trials used more and were not plotted</span>}
                            </div>
                        ) : null}
                    </div>
            ...
            <div ref={chartRef} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
                ...
                >
                    ...
                    <AnimatedAxis
                        ...
                        tickValues={tickVals}
                        tickFormat={bottomTickFormat as any}
                        tickLabelProps={() => ({
                            fill: hoverColor,
                            fontSize: 11,
                    ...
                    <Tooltip
                        ...
                        renderTooltip={() => {
                                ...
                                if (fallbackSeries === 0 || labels[fallbackSeries] === "Overall") {
                                    ...
                                    return (
                                        <div style={{ color: "var(--text-primary)" }}>
                                            ...
                                            <div>
                                                {weekNumber === 0 ? "Right now," : `In ${weekNumber} weeks,`} you have a {(yValue * 100).toFixed(1)}% chance of
                                                success overall
                                            </div>
                                        </div>
                                    )
                                    ...
                                    return (
                                        <div style={{ color: "var(--text-primary)" }}>
                                            ...
                                            <div>
                                                {weekNumber === 0 ? "Right now," : `In ${weekNumber} weeks,`} you have a {(yValue * 100).toFixed(1)}% chance of
                                                having enough {labels[fallbackSeries]}
                                            </div>
                                        </div>
                                    )
                                }
                                ...
                                if (fallbackSeries === 0) {
                                    ...
                                    return (
                                        <div style={{ color: "var(--text-primary)" }}>
                                            <div style={{ color: effectiveColors[fallbackSeries], fontWeight: 600 }}>{labels[fallbackSeries]}</div>
                                            <div>
                                                {weekNumber === 0 ? "Right now," : `In ${weekNumber} weeks,`} this will cost {formatSig(yValue)} (buying needed
                                                mats with {formatSig(costToPity - srcCounts[5][hoverBucket])} Gold)
                                            </div>
                                        </div>
                                    )
                                    ...
                                    if (Math.abs(goldFromSell - goldBudget) < 1e-9) {
                                        return (
                                            <div style={{ color: "var(--text-primary)" }}>
                                                <div style={{ color: effectiveColors[fallbackSeries], fontWeight: 600 }}>{labels[fallbackSeries]}</div>
                                                <div>
                                                    {weekNumber === 0 ? "Right now," : `In ${weekNumber} weeks,`} you will have {formatSig(goldBudget)} Gold
                                                    (nothing to sell)
                                                </div>
                                            </div>
                                        )
                                        ...
                                        return (
                                            <div style={{ color: "var(--text-primary)" }}>
                                                <div style={{ color: effectiveColors[fallbackSeries], fontWeight: 600 }}>{labels[fallbackSeries]}</div>
                                                <div>
                                                    {weekNumber === 0 ? "Right now," : `In ${weekNumber} weeks,`} you will have {formatSig(goldFromSell)} Gold
                                                    if you sell mats, {formatSig(goldBudget)} if you don't
                                                </div>
                                            </div>
                                        )
                                    }
                                    ...
                                    return (
                                        <div style={{ color: "var(--text-primary)" }}>
                                            <div style={{ color: effectiveColors[fallbackSeries], fontWeight: 600 }}>{labels[fallbackSeries]}</div>
                                            <div>
                                                {weekNumber === 0 ? "Right now," : `In ${weekNumber} weeks,`} you need to spend {formatSig(yValue)} on buying{" "}
                                                {labels[fallbackSeries]}
                                            </div>
                                        </div>
                                    )
                                }
                            ...
                            const srcCounts = effectiveCounts ?? counts
                            const cumPctRaw =
                                (srcCounts[fallbackSeries].slice(0, hoverBucket + 1).reduce((partialSum, a) => partialSum + a, 0) / data_size) * 100
                            ...
                            return (
                                <div style={{ color: "var(--text-primary)" }}>
                                    ...
                                    <div>
                                        {cumPct} used less than {formatSig(mid, 3)} {labels[fallbackSeries]}
                                    </div>
                                </div>
                            )
                        }}
