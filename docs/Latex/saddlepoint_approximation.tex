...
Let $U'_j = U^*_j$ if $j \leq k$, otherwise $U'_j = U_j$ .
...
$$
So $Y_i$ is the random variable representing the cost of material type $i$. We can notice that :
$$
...
Where part $a)$ is trivial to calculate because $Y$ is a simple sum of RVs, making $\mathbb{E}$ distributive.

For part $b)$, we define the biased distribution $Y^*$, which has the probabily distribution $p^* = p \cdot \frac{y}{\mathbb{E}[Y]}$ for each $y$ in the support of $Y$. In other words:
...
w &=  \text{sgn}(\theta)\sqrt{2 (sb^+-K(\theta))}, \\
 u &= 2  \delta^{-1}\sinh(s\delta/2)\sqrt{K''(\theta)}, \\
 z &= (b^+ - \mu)/\sigma,\\
...
The derivation of this formula can be found in Saddlepoint Approximations with Applications \cite{butler2007saddlepoint}. Specifically, the base form is stated in Section 1.2.1, the continuity corrected version in Section 2.4.4, and the limiting case in Section 5.2.3.
