...
To describe this in English, this would be ``Average gold cost incurred due to needed mats''. Making use of the notations we've built up, this is
$$
...
The derivation of this formula can be found in Chapter 1 and 2 of Saddlepoint Approximations with Applications \cite{butler2007saddlepoint}. Specifically, the base form is stated in Section 1.2.1, the continuity corrected version in Section 2.4.4, and the limiting case in Section 5.2.3.
...
$$
K_{Y^*}(s) = K_Y(s) + \log(K'_Y(s)) - \log(\mu).
$$
