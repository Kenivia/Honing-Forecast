...
import { FLOAT_TOL, GRAPH_FONT_SIZE, GRAPH_HEIGHT } from "@/_utils/_constants";
import { cssVar } from "@/_utils/_helpers";
    ...
    return props.annotations
        ...
        .map((xVal, i) => {
            ...
            if (positionStr === "graph")
                textY = clamp(svgY - GRAPH_FONT_SIZE, GRAPH_FONT_SIZE + 4, GRAPH_HEIGHT - GRAPH_FONT_SIZE);
            return {
                xVal,
                svgX,
                svgY,
...
<template>
    <div ref="rootRef" class="hf-material-graph" @mousemove="onMouseMove" @mouseleave="onMouseLeave">
        <svg :width="Math.max(width, MIN_GRAPH_WIDTH)" :height="height" role="img" aria-label="Material graph">
            <defs>
                <linearGradient
                    :id="`hf-graph-fill-${graphColor.replace(/[^a-z0-9]/gi, '')}`"
                    x1="0"
                    y1="0"
                    x2="0"
                    y2="1"
            ...
            <g>
                ...
                <path
                    v-if="!isEmpty"
                    :d="areaPath"
                    :fill="`url(#hf-graph-fill-${graphColor.replace(/[^a-z0-9]/gi, '')})`"
                    stroke="none"
                />
                ...
                <g v-if="hoveredPoint">
                    ...
                    <circle
                        ...
                        r="4"
                        fill="var(--hf-graph-hover-color, #ffffff)"
                    />
                </g>
