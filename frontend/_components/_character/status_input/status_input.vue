...
import {
    ...
    pending_ilevel,
} from "@/_utils/_helpers";
import { computed } from "vue";
import { ADV_COLS, ALL_LABELS, NORMAL_COLS, NUM_PIECES, PLUS_TIER_CONVERSION } from "@/_utils/_constants";
import TierConvertButton from "@/Components/Common/TierConvertButton.vue";
import { UpgradeStatus } from "@/_utils/_interfaces";
import { grid_change_callback } from "../char_worker_util";
import { useRosterStore } from "@/_stores/roster_config";
import { input_column_to_num, parse_input } from "@/_utils/input_column";
...
function change_tier() {
    ...
    stay_same_indices.forEach(
        (index) =>
            (active_profile.value.bound_budgets[new_tier].data[index] =
                active_profile.value.bound_budgets[old_tier].data[index]),
    );
    ...
    active_profile.value.bound_budgets[new_tier].data[new_index] =
        active_profile.value.bound_budgets[old_tier].data[old_index];
