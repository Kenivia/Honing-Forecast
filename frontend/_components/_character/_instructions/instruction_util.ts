...
import { DEFAULT_ARTISAN_MULTIPLIER, JOINED_ADV_JUICE } from "@/_utils/_constants";
import { input_column_to_num } from "@/_utils/input_column";
import { Upgrade } from "@/_utils/_interfaces";
...
export function streaks_to_text(
    upgrade: Upgrade,
    streaks: NormalStreak[] | AdvStreak[],
    juice_info: any,
    alr_done: number,
) {
    let out = [];
...
export function artisan_function(upgrade: Upgrade, total_count: number, juice_info: any): string {
    ...
    for (let count = 0; count < total_count; count++) {
        ...
        let current_chance = Math.min(
            1,
            upgrade.base_chance + upgrade.extra_chance + min_count * upgrade.base_chance * 0.1 + extra_arr[count],
        );
        if (artisan >= 1.0) {
            break;
        }
...
export function compute_remaininig_materials(used_materials: number[]) {
    ...
    used_materials.forEach((cost, index) => {
        if (cost <= 0) {
            bound_budgets.push(
                input_column_to_num(active_profile.value.bound_budgets[active_profile.value.tier])[index],
            );
