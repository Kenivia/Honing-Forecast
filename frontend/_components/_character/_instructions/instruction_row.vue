...
import { ALL_LABELS, T4_JUICE_LABELS } from "@/_utils/_constants";
import { get_piece_name, get_icon_path } from "@/_utils/_helpers";
import { Upgrade, UpgradeStatus } from "@/_utils/_interfaces";
            ...
            <div class="hf-popup" @click.stop>
                <div v-if="upgrade.is_normal_honing" class="popup-header">
                    ...
                    <div class="input-row text-left">
                        ...
                        {{ artisan_function(upgrade, Math.max(0, taps_so_far - 1), juice_info) }}%
                    </div>
                    <div class="input-row text-left">
                        Cumulative chance:
                        {{ cumulative_chance(upgrade, taps_so_far, juice_info) }}%
...
.order-circle {
    width: calc(var(--icon-size) * 2 + 0.25rem);
