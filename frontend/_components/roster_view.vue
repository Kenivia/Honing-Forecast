...
import { CharProfile, create_default_char_profile, recreate_char_profile } from "@/_stores/character_profile";
import { create_default_owned_input_column, useRosterStore } from "@/_stores/roster_config";
import { achieved_ilevel, format_char_name, pending_ilevel } from "@/_utils/_helpers";
...
<template>
    <Sidebar header="Roster setup">
        ...
        <template #main>
            <div :class="'hf-main-stage ' + (roster_ids.length > 1 ? 'hf-outer-budget-grid' : 'hf-only-one-roster')">
                <section v-for="(roster_id, roster_index) in roster_ids" class="hf-card" :key="roster_id">
                    ...
                    >
                        ...
                        <button class="hf-header-button" @click="() => duplicate(profile_index)">Make a copy</button>
                        <button
