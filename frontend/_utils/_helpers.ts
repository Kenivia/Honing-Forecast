import { CharProfile } from "@/_stores/character_profile";
import { ADV_COLS, IconMap, NUM_PIECES, PIECE_NAMES, PLUS_TIER_CONVERSION } from "./_constants";
import { Upgrade, UpgradeStatus } from "./_interfaces";
...
export function has_upgrades_in_range(low: number, high: number, is_weapon: boolean, is_adv: boolean) {
    ...
    for (let row = is_weapon ? 5 : 0; row < (is_weapon ? NUM_PIECES : NUM_PIECES - 1); row++) {
        for (let col = low - 1; col < high; col++) {
            if (
                (is_adv ? active_profile.value.adv_grid : active_profile.value.normal_grid)[row][col] ===
                UpgradeStatus.Want
            ) {
                return true;
            }
...
export function format_char_name(raw: string, char_index: number, profiles?: CharProfile[]): string {
    const all_profiles =
        profiles === null || profiles === undefined ? storeToRefs(useRosterStore()).all_profiles.value : profiles;
...
export function pending_ilevel(active_profile: CharProfile): string {
    ...
    if (active_profile.tier == 0) {
        for (let row = 0; row < NUM_PIECES; row++) {
            let highest_plus =
                active_profile.adv_grid[row].findLastIndex(
                    (value) => value == UpgradeStatus.Done || value == UpgradeStatus.Want,
                ) + 1;
            out += (highest_plus * 10) / 6;
        }
    ...
    for (let row = 0; row < NUM_PIECES; row++) {
        let highest_plus =
            active_profile.normal_grid[row].findLastIndex(
                (value) => value == UpgradeStatus.Done || value == UpgradeStatus.Want,
            ) + 1;
        // find last index = -1 if nothing is done
        if (highest_plus == 0) {
