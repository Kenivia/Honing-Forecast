import { ALL_LABELS, DEFAULT_TIER, STORAGE_KEY } from "@/_utils/_constants";
import { debounce, format_char_name } from "@/_utils/_helpers";
import { create_input_column, validate_input_column, validate_input_column_array } from "@/utils/input_column";
import { InputColumn, InputType } from "@/_utils/_interfaces";
import { defineStore } from "pinia";
import { CharProfile, create_default_char_profile, recreate_char_profile } from "./character_profile";
import { get_valid_status_grid } from "@/utils/status_grid";
import { grids_to_keyed } from "@/utils/keyed_upgrades";
...
export function write_roster_config(state) {
    try {
        localStorage.setItem(
            STORAGE_KEY + "_roster",
            stringifyOmit(state.roster_config, ["optimizer_worker_bundle", "histogram_worker_bundle"]),
        );
    } catch {
