...
export function aggregate_streaks(
  upgrade: Upgrade,
  juice_info: any,
  alr_done: number,
): NormalStreak[] | AdvStreak[] {
  ...
  if (upgrade.is_normal_honing) {
    ...
    let index = alr_done;
    for (const [juice, book] of upgrade.state.slice(
      alr_done,
      upgrade.normal_dist.length - 1,
    )) {
      if (
        index == upgrade.normal_dist.length - 2 &&
        artisan_function(upgrade, index, juice_info) === "100.00"
      ) {
    ...
    let [scroll_grace, scroll_non_grace] =
      JOINED_ADV_JUICE[upgrade.state[1][1]];
