...
function change_tier() {
  ...
  active_profile.value.special_budget.data[0] = parse_input(
    ...
    0,
    String(
      input_column_to_num(active_profile.value.special_budget, true)[0] *
        multiplier,
    ),
    true,
  ).toLocaleString();
      ...
      <div class="card-header">
        <div class="fler-row flex gap-5">
          <label class="text-(--achieved)"
            >Achieved ilevel: {{ achieved_ilevel(active_profile) }}</label
          >
          <label class="text-(--pending)"
            >Pending ilevel: {{ pending_ilevel(active_profile) }}</label
          >
        </div>
        <TierConvertButton
