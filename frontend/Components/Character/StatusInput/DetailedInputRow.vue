...
<template>
    <input v-if="upgrade.is_normal_honing" type="number" v-model.number="artisan" min="0" :max="99.99" @change="write_progress()" />
    <div v-if="!upgrade.is_normal_honing">
        ...
        <div class="input-row">
            ...
            <input type="number" v-model.number="current_adv_xp" min="0" max="90" step="10" style="justify-self: flex-start" @change="write_progress" />
        </div>
        <div class="input-row grid-4">
            ...
            <label v-if="current_grace_progress === 0 && (current_adv_xp > 0 || current_adv_upgrade > upgrade.upgrade_index * 10)" class="check-label">
