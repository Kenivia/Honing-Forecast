...
import { Payload } from "./payload_builder";
import { HistogramOutputs, StateBundle, WasmOp } from "@/_utils/_interfaces";
    ...
    function _launch(wasm_op: WasmOp, payload: Payload, cancel: boolean, callback?: (result) => void) {
        ...
        worker.onmessage = (e) => {
            ...
            if (e.data.type === "result") {
                ...
                if (
                    performance.now() - last_intermediate_time.value > 1000 ||
                    est_progress_percentage.value < 0.000001
                ) {
