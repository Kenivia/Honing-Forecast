...
export function getNextUnfinishedIndex(upgradeArr: Upgrade[], excludeIndex?: number): number {
    let first_try = upgradeArr.findIndex((z) => z == sortedUpgrades(upgradeArr).find((upg, i) => !upg.succeeded && i > excludeIndex))
    // console.log(first_try)
    if (first_try < 0) {
        return upgradeArr.findIndex((z) => z == sortedUpgrades(upgradeArr).find((upg, i) => !upg.succeeded && i !== excludeIndex))
    }
    return first_try
}
...
//     const taps = upgrade.alr_failed ?? 0
