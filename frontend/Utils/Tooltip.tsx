...
export function createTooltipHandlers(setTooltip: React.Dispatch<React.SetStateAction<TooltipState>>) {
    ...
    const showUpgradeTooltip = (upgrade: any, costLabels: string[], tapRecordCosts: number[], x: number, y: number) => {
        setTooltip({
            visible: true,
            type: "upgrade",
            x: x + 10,
...
export function renderTooltip(tooltip: TooltipState, mainScale: number = 1, zoomCompensation: number = 1) {
    ...
    if (tooltip.type === "separator") {
        return <div style={tooltipStyle}>{tooltip.content}</div>
    }
            ...
            <div style={tooltipStyle}>
                <div style={containerStyle}>
                    <div style={leftColumnStyle}>
                        ...
                        {upgrade.is_finished && upgrade.current_artisan && <div>Artisan: {(upgrade.current_artisan * 100).toFixed(2)}%</div>}
                        ...
                        {upgrade.is_normal_honing && upgrade.is_finished && (
                            <div>In a room of 100 people, you are luckier than {(100 - (upgrade.cumulative_chance || 0) * 100).toFixed(0)} of them.</div>
                        )}
                    </div>
                    <div style={rightColumnStyle}>
                        <div>
                            <strong>Costs so far(excluding unlock):</strong>
                        </div>
