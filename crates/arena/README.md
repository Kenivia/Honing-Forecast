...
This is where the optimizer engine is tested. You can add another version in the engine folder in core, add it in `mod.rs` and both`Cargo.toml` (I know it's tedious but idk how else to do it), then test it via `cargo run --package hf-arena --bin hf-arena --no-default-features --features run_tests YOUR_VERSION  --profile release -- ./test_payloads` (This should be ran from the project root).
...
- remember to call upgrade.update_hash
