...
use crate::saddlepoint_approximation::normal_sa::{init_dist, new_prob_dist};
...
pub fn apply_price_leftovers(
    ...
    let mut mats_gold = vec![0.0; avg_mats.len()];
    let mut juice_gold = vec![(0.0, 0.0); avg_juices.len()];
    for (index, gold) in mats_gold.iter_mut().enumerate() {
        if prep_output.budgets[index] as f64 > avg_mats[index] {
            *gold = (prep_output.budgets[index] as f64 - avg_mats[index])
                * prep_output.leftover_values[index]
        } else {
            *gold =
                (prep_output.budgets[index] as f64 - avg_mats[index]) * prep_output.price_arr[index]
        }
    }

    for (id, (weap, armor)) in juice_gold.iter_mut().enumerate() {
        let weap_diff: f64 = prep_output.juice_books_owned[id].0 as f64 - avg_juices[id].0;
        let armor_diff: f64 = prep_output.juice_books_owned[id].1 as f64 - avg_juices[id].1;
        if weap_diff > 0.0 {
