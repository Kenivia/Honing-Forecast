...
use crate::verification::monte_carlo::verify_result_with_monte_carlo;
use chrono::Local;
use rand::prelude::*;
use rayon::prelude::*;
        ...
        |(test_case_name, state_bundle, metric_type_string, metric_type_num, trial_num)| {
            ...
            if *trial_num == 1 {
                let mc_result = verify_result_with_monte_carlo(
                    ...
                    MONTE_CARLO_PRECISION * state_bundle.metric.abs(),
                    MONTE_CARLO_COUNT,
                    &mut state_bundle,
                    &mut rng,
                );
                let monte_carlo_mean =
                    if metric_type_string == "SA" || metric_type_string == "brute" {
                ...
                let seen = if seen_tests.contains_key(&key) {
                    seen_tests[&key]
                } else {
                    NAN
                };
                if is_verify && seen_tests.contains_key(&key) {
                    if my_pct_diff(seen, state_bundle.metric) > SIMULATED_ANNEALING_DIFF_TOL {
                        dbg!(&mc_result.prob_leftover);
                        panic!(
                            "Previous result and SA didn't fall within +-{}%, confidence {}%, n = {}.
                            {} Test: {} Trial: {} prev: {} new: {} diff w/ prev: {}%",
                            MONTE_CARLO_PRECISION * 100.0,
                            MONTE_CARLO_CONFIDENCE * 100.0,
                            mc_result.samples,
                            ACTIVE_FEATURE,
